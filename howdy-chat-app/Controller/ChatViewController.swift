...
class ChatViewController: UIViewController {
    ...
    @IBAction func sendButtonTapped (_ sender: Any) {
        ...
        DatabaseService.instance.uploadPost(withMessage: message, forGroupKey: (group?.key)!) { (success) in
            if !success
            {
                self.handleError(message: "Message not delivered. Please try again.")
            }
            else
            {
...
extension ChatViewController {
    ...
    func handleError( message: String ) {
        present(errorAlert(title: "Message Failure.", message: message), animated: true, completion: nil)
    }
...
extension ChatViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func uploadToStorageAndCreateMediaPost ( withNSURL url: NSURL) {
        ...
        StorageService.instance.uploadVideoToStorage(withURL: url, andFolderKey: SK_MESSAGE_VID) { (videoUrl) in

            let message = Message(senderId: (Auth.auth().currentUser?.uid)!, type: self.MessageType.Video.rawValue, time: String(Int(NSDate().timeIntervalSince1970)), content: videoUrl)
